...
{
    // how do you make a function that takes 2 paremters and always has a unique output
	char xc = (char)x;
	char yc = (char)y;
	char final = xc+yc;
	return (int)final;
...
void remove_all(node **head)
{
    while (*head != NULL)
    { 
        node * prevhead = remove_first(head);
		free(prevhead);
    }
}
...
{
	// literally just checking if the coords are equal 
	// invalid read 
		// probably has to do with (*head) 
	//return (head->host.x==host.x && head->host.y==host.y);
	while (head != NULL)
	{
...
int one_round(THost *hosts, int m, node *p_arr[], int n_arr, int k, int T)
{
    ...
    for(int i = 0; i < m; i++)
    {
		...
		// i dont use the variable T i believe is hould be used below but how not sure
