 ...
 * finish.
 *
 * This function needs to fork, and then upgrade
...
void start_program(Program *programs, int num_programs, int cur) 
{
    ...
    {
        // now time to upgrade child process
        // this should be the exec call
        dup2(programs[cur].fd_in, 0);
        dup2(programs[cur].fd_out, 1);
        for(int i = cur+1; i < num_programs-1; i++) // the bounds are so we do not close the actual stdin and stdout
        {
    ...
    {
        programs[cur].pid = id;
        // if blocks are to make sure we do not close actual stdin and stdout
        if (cur != 0)
        {
...
{
    // TODO
    // it might be easier to use both this function and the syart program funciton
    // here i would set the input and output locations for all functions using pipies

    // what if we have a loop where we create a pipe assign the the I/O and then close them and repeat, 
    //      would this create new pipes or bug out
    // PIPEFD_READ  = 0
    // PIPEFD_WRITE = 1
    programs[0].fd_in = 0;
    for (int i=0; i < num_programs-1; i++)
    {
        ...
        programs[i].fd_out = topipe[1];
        programs[i+1].fd_in = topipe[0];
    }
