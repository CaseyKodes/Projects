...
{
    // TODO
    printer_t * printer_data = (printer_t *) arg;
    job_queue_t * jq = printer_data->jq;
    ...
    while (1) {
        ...
        int job = q_fetch_job(jq, printer_data->id);
        pthread_mutex_unlock(&jq->mutex);
...
int main(int argc, char *argv[])
{
    ...
    for (j = 0; j < num_printers; j++) {
        printers[j].id = j;
        printers[j].jq = &job_queue;
        printers[j].njobs = 0;
        started = pthread_create(&printers[j].thread_id, NULL, printer_main, &printers[j]);
    ...
    for (j = 0; j < num_printers; j++) {
        started = pthread_join(printers[j].thread_id, NULL);
