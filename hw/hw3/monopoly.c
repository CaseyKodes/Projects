...
int one_round(int m, int n, TPlayer p[], TProperty prop[])
{
	for(int i = 0; i < m; i++)
	{
		...
		/*
		ROADBLOCKS
			-how to handle when a player rolls a number that is higher than 
			the number of remaining tiles on the board
			ANSWER spot they go to is equal to (location+steps)-n
				we also then need to add n to that players balance 
			-do player have to pay to get the property when it has no owner
			ANSWER no
			- pointers in the second else statment are not working properly
			thought casting would correct but it does not
		*/
		// calculating landing space 
		if (p[i].location+steps>=n)
		{
			p[i].location = p[i].location+steps-n; 
			p[i].balance += n; // add n to balance after passing or landing on 0
		}
		else 
		{
			p[i].location = p[i].location+steps;
		}
		// calculating how to interact with the property
		if(prop[p[i].location].owner_id == -1)
		{
			prop[p[i].location].owner_id = p[i].id;
		}
		else 
		{
			// if i can make the first and second arguments for this call to transaction pointers than I think this whole file is done
			int toReturn = transaction(&p[i], &p[prop[p[i].location].owner_id], prop[p[i].location].rent);
			if (toReturn==0) // only if the trsanaction is not valid should we exit the fuction
			{
				return toReturn;
			}
		}
