...
void writeint(int filedes, int v)
{
    write(filedes, &v, sizeof(int));
}
void readint(int filedes, int * pv)
{
    read(filedes, pv, sizeof(int));
}
...
void child_main(int fdp[], int fdc[], int seed)
{
    ...
    // close parts of pipes that parent uses since in child
    close(fdp[1]);
    close(fdc[0]);
    // send max value to parent
    writeint(fdc[1], MAX_VALUE);
    int guess;
    int result;
    do 
    {
        readint(fdp[0], &guess);
        result = gmn_check(&gmn, guess);
        writeint(fdc[1], result);
    } while (result != 0);

    // how do you send the final message back?
...
int main(int argc, char *argv[])
{
    ...
    // in the parent closing parts of pipes that c hild uses
    close(fdp[0]);
    close(fdc[1]);
    //getting max from child
    readint(fdc[0], &max);
    do { 
        guess = (min + max)/2;
        printf("My guess: %d\n", guess);
        ...
        writeint(fdp[1], guess);
        readint(fdc[0], &result);
        if (result > 0)
            min = guess + 1;
        else if (result < 0)
            max = guess - 1;
    ...
    // how to recieve final message?
    char toprint[MSG_BUF_SIZE] = "";
    read(fdc[0], toprint, MSG_BUF_SIZE);
    //print final message 
    printf(toprint);
