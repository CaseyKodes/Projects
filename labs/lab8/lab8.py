class CustomSet:
    ...
    def _rehash(self, new_buckets):
        ...
        2*n_buckets or 1/2*n_buckets, depending on whether we are reahshing up or down."""
        # Make a new list of `new_buckets` empty lists
        self._n_buckets = new_buckets
        new_list = [[] for i in range(self._n_buckets)]
        ...
        for bucket in self._L:
            ...
            for item in bucket:
                ...
                newIDX = self._find_bucket(item)
                # add that item to the correct bucket
                new_list[newIDX].append(item) 
