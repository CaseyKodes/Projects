...
// happens. Do we always get the result sum?
    // when running the code before modification we do not always get the same output from the same inputs
...
void* calculate(void* threadarg)
{
    ...
    if(n > 0)
    {
        ...
        //this line helps us to see wrong results
            // to get aorund this messing up the code we just need to use mutex
        sleep(0);
        n--;
    }
